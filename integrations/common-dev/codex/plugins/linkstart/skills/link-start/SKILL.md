---
name: link-start
description: Connects an interactive HTML or localhost app back to the same Claude
  Code session or Codex thread that produced it through LinkStart v1 Preview. Use
  when the user wants an agent-generated app to keep exchanging events and feedback
...
Use only the bundled Runtime under `assets/bin/<target>/`; run `scripts/runtime.py` to resolve, verify, and launch it. Require Runtime `0.1.1`, protocol `v1`, exact v0.1.1 release provenance, SHA-256, size, and Unix executable mode from `assets/checksums.json`. Never download, compile, copy elsewhere, or fall back to `PATH`.
...
2. **Origin attach/rebind** — apply the selected host reference, prove this is the current Origin Session, establish or rebind one Agent Connection, and retain its bearer privately. Callsign is display-only; `connectionId` is a locator, not authentication.
...
4. **Monitor** — arm the selected host's wait mechanism. Accept one in-flight Event at a time, acknowledge only after adapter acceptance, send durable Feedback when appropriate, and re-arm using the host-specific rhythm.
