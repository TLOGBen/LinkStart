...
export function createLinkStartClient({ baseUrl }) {
  ...
  return {
    async redeem(grant) { const x=await unwrap(fetch(`${baseUrl}/v1/launch-grants/redeem`,{method:"POST",headers:{Authorization:`Bearer ${grant}`}})); instanceId=x.instanceId;capability=x.capability;return {instanceId,status:x.status}; },
